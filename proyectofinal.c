...
>>>>>>> feature/visuales-zombie
  ...
  void gameplay(struct Jugador *jugador, int dificultad) {
        ...
        // lo siguiente definira el tamnho de la matriz de acuerdo a la dificultad que se obtine, en este caso se hace mediante el switch donde case 1 es dificl y case 3 ez
        system("clear");
        ...
        // aqui lo que se hace es que la matriz se pinta, donde la pos del juegador tiene un print diferente que es una carita para diferenciar y donde el personaje ya paso es un asterisco, este  bucle hace que cada vez que suceda algo en la matriz se vuelva a pintar completa
        while (1) {
            system("clear");
            for (int i = 0; i < filas; i++) {
  for (int j = 0; j < columnas; j++) {
            ...
            // esto es de lo mas importante del codigo, ya que dependiendo del caracter de la matriz donde el personaje cae se llama a la funcion de combate, jefe o tienda que tiene logica diferentes para uno de los 3 casos c,t,j
...
//aqui basicamente definimos la probabilidad del enemigo y jugador dependiendo de la dificultad que se pasa por parametro
